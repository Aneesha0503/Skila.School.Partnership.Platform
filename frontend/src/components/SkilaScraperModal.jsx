  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-900/60 dark:bg-slate-950/80 backdrop-blur-xs flex items-center justify-center p-3 sm:p-6 overflow-y-auto">
      <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-2xl max-w-4xl w-full max-h-[90vh] flex flex-col overflow-hidden animate-in fade-in duration-200">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 text-xs">
              <div>
                ...
                <input
                  ...
                  className="w-full p-2 border border-slate-300 dark:border-slate-700 rounded-lg bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100 focus:ring-1 focus:ring-indigo-500"
                />
              </div>
              <div>
              ...
              <div>
                <label className="block text-slate-700 dark:text-slate-300 font-semibold mb-1">Mandal / Sub-Region</label>
                <input
                  type="text"
                  value={mandal}
              ...
              <div>
                <label className="block text-slate-700 dark:text-slate-300 font-semibold mb-1">Schools to Discover</label>
                <select
                  value={count}
                  onChange={(e) => setCount(e.target.value)}
                  className="w-full p-2 border border-slate-300 dark:border-slate-700 rounded-lg bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100 font-medium"
                >
            ...
            <div className="flex flex-col sm:flex-row gap-2 items-center">
              <div className="relative flex-1 w-full">
                ...
                <input
                  ...
                  className="w-full pl-9 pr-3 py-2 text-xs border border-slate-300 dark:border-slate-700 rounded-lg bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100 focus:ring-1 focus:ring-indigo-500"
                />
              </div>
              ...
              <button
                type="submit"
                disabled={isScraping}
                className="w-full sm:w-auto inline-flex items-center justify-center gap-2 px-5 py-2 rounded-lg bg-gradient-to-r from-indigo-600 to-violet-600 hover:from-indigo-700 hover:to-violet-700 text-white text-xs font-semibold shadow-sm transition shrink-0 cursor-pointer disabled:opacity-50"
              >
                {isScraping ? (
                  <>
        ...
        {/* Results Area */}
        <div className="p-5 overflow-y-auto flex-1 bg-white dark:bg-slate-900">
          {isScraping && (
            <div className="py-16 flex flex-col items-center justify-center text-center">
              <div className="w-16 h-16 rounded-2xl bg-indigo-50 dark:bg-indigo-950/60 border border-indigo-100 dark:border-indigo-800 flex items-center justify-center text-indigo-600 dark:text-indigo-400 mb-4 animate-pulse">
                ...
                {scrapedSchools.map((s) => {
                  ...
                  return (
                    <div
                      ...
                      className={`p-4 rounded-xl border text-xs transition cursor-pointer flex flex-col sm:flex-row items-start justify-between gap-3 ${isChecked ? 'bg-indigo-50/40 dark:bg-indigo-950/40 border-indigo-300 dark:border-indigo-700 ring-1 ring-indigo-200 dark:ring-indigo-800' : 'bg-slate-50 dark:bg-slate-800/40 border-slate-200 dark:border-slate-800 opacity-70'}`}
                    >
                      <div className="flex items-start gap-3">
                        ...
                        <div>
                          <div className="flex items-center gap-1.5 flex-wrap mb-1">
                            <span className="font-mono text-[10px] px-1.5 py-0.5 rounded bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-300 font-semibold border border-slate-200 dark:border-slate-700">
                              UDISE: {info?.udise_code}
                            </span>
                            <span className="text-[10px] px-1.5 py-0.5 rounded bg-indigo-100 dark:bg-indigo-950/60 text-indigo-800 dark:text-indigo-300 font-semibold">
                              {info?.board}
                            </span>
                            <span className="text-[10px] px-1.5 py-0.5 rounded bg-rose-50 dark:bg-rose-950/60 text-rose-700 dark:text-rose-300 border border-rose-100 dark:border-rose-800 font-bold">
                              {sales?.skila_ai_potential} AI Potential
                            </span>
                          </div>

                          <h4 className="font-bold text-slate-900 dark:text-white text-sm mb-1">{info?.school_name}</h4>
                          <p className="text-slate-500 dark:text-slate-400 flex items-center gap-1 text-[11px] mb-2">
                            <MapPin className="w-3 h-3 text-slate-400 dark:text-slate-500" />
                            {hierarchy?.village_locality_ward}, {hierarchy?.mandal} Mdl, {hierarchy?.district}, {hierarchy?.state}
                          </p>

                          <div className="flex items-center gap-2 flex-wrap text-[10px] text-slate-600 dark:text-slate-300">
                      ...
                      <div className="sm:text-right shrink-0 mt-2 sm:mt-0">
                        ...
                        <span className="font-bold text-emerald-700 dark:text-emerald-400">{sales?.annual_fee_range}</span>
                      </div>
                    </div>
                  );
                })}
          ...
          <div className="flex items-center gap-2">
            ...
            {scrapedSchools.length > 0 && (
              <button
                ...
                className="inline-flex items-center gap-1.5 px-5 py-2 text-xs font-semibold text-white bg-indigo-600 hover:bg-indigo-700 rounded-lg transition shadow-xs cursor-pointer disabled:opacity-50"
              >
